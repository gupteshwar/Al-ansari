...
def update_data(file_path, doc):
	...
	if not latest_record:
		...
		if doc == 'Purchase Invoice':
			...
			frappe.db.commit()
			return 'Yes'
		elif doc == 'Sales Invoice':
			data = pd.read_csv((latest_record[0][0]).replace(" ", "%20"))
			df = pd.DataFrame(data)
			for i, row in df.iterrows():
				if frappe.db.exists('Sales Invoice', row['ID']):
					get_doc = frappe.get_doc('Sales Invoice', row['ID'])
					get_doc.db_set("orion_reference_id", row['Orion Reference ID'])
				else:
					pass
			frappe.db.commit()
