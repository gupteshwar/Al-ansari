...
def validate_cost_center(doc, method):
    if doc.cost_center:
        for i in doc.items:
            ...
            elif i.against_sales_order and not i.against_sales_invoice:
            ...
            elif i.against_sales_invoice and not i.against_sales_order:
                sales_invoice = frappe.get_doc('Sales Invoice', i.against_sales_invoice)
                for so in sales_invoice.items:
                    if i.item_code == so.item_code and i.idx == so.idx:
                        i.cost_center = so.cost_center
            elif i.against_sales_invoice and i.against_sales_order:
                sales_order = frappe.get_doc('Sales Order', i.against_sales_order)
                for so in sales_order.items:
                    if i.item_code == so.item_code and i.idx == so.idx:
                        i.cost_center = so.cost_center
